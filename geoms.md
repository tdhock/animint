...
data$y <- rnorm(length(data$x), data$x, 0.5) + rep(rnorm(5, 0, 2), each = 10)
...
p3 <- ggplot() + geom_line(data = data, aes(x = x, y = y, colour = group, group = group, 
    linetype = lt)) + ggtitle("geom_line + scale_linetype_manual")
p3
```
...
p4 <- ggplot() + geom_line(data = data, aes(x = x, y = y, colour = group, group = group, 
    linetype = group)) + ggtitle("geom_line + scale_linetype automatic")
p4
```
...
p5 <- ggplot() + geom_line(data = data, aes(x = x, y = y, colour = group, group = group, 
    linetype = lt)) + scale_linetype_identity("group", guide = "legend", labels = c("1", 
    "2", "3", "4", "5")) + scale_colour_discrete("group") + ggtitle("Manual Linetypes: dash-space length")
p5
```
...
lt1 <- data.frame(x = 0, xend = 0.25, y = 1:13, yend = 1:13, lt = lts, lx = -0.125)
p6 <- ggplot() + geom_segment(data = lt1, aes(x = x, xend = xend, y = y, yend = yend, 
    linetype = lt)) + scale_linetype_identity() + geom_text(data = lt1, aes(x = lx, 
    y = y, label = lt), hjust = 0) + ggtitle("Scales package: all linetypes")

lts2 <- c("solid", "dashed", "dotted", "dotdash", "longdash", "twodash")
lt2 <- data.frame(x = 0, xend = 0.25, y = 1:6, yend = 1:6, lt = lts2, lx = -0.125)
p7 <- ggplot() + geom_segment(data = lt2, aes(x = x, xend = xend, y = y, yend = yend, 
    linetype = lt)) + scale_linetype_identity() + geom_text(data = lt2, aes(x = lx, 
    y = y, label = lt), hjust = 0) + ggtitle("Named linetypes")
...
data2$y <- rnorm(length(data2$x), data2$x * rep(rnorm(n, 1, 0.25), each = pts), 
    0.25) + rep(rnorm(n, 0, 1), each = pts)
data2$lty <- "solid"
data2$lty[which(data2$group %in% subset(data2, x == 10)$group[order(subset(data2, 
    x == 10)$y)][1:floor(n/10)])] <- "3133"
data2 <- ddply(data2, .(group), transform, maxy = max(y), miny = min(y))
data2$below0 <- factor(sign(data2$miny) < 0)

qplot(data = data2, x = x, y = y, group = group, geom = "line", alpha = I(0.2))
```
...
#' scale_alpha
p8 <- ggplot() + geom_line(data = data2, aes(x = x, y = y, group = group), alpha = 0.1) + 
...
p9 <- ggplot() + geom_line(data = subset(data2, as.numeric(group) < 50), aes(x = x, 
    y = y, group = group, linetype = lty), alpha = 0.2) + scale_linetype_identity() + 
...
p10 <- ggplot() + geom_line(data = subset(data2, as.numeric(group) < 50), aes(x = x, 
    y = y, group = group, linetype = below0, alpha = maxy)) + scale_alpha_continuous(range = c(0.1, 
    0.5)) + ggtitle("Continuous alpha")
p10
```
...
p11 <- ggplot() + geom_line(data = subset(data2, as.numeric(group)%%50 == 1), 
    aes(x = x, y = y, group = group, size = (floor(miny) + 3)/3)) + scale_size_continuous("Line Size", 
    range = c(1, 3)) + ggtitle("Continuous size")
p11
```
...
p12 <- ggplot() + geom_line(data = data2, aes(x = x, y = y, group = group, alpha = miny, 
    colour = maxy)) + scale_alpha_continuous(range = c(0.1, 0.3)) + ggtitle("Continuous Alpha and Colour")
p12
```
...
scatterdata$class4 <- factor(rowSums(sapply(quantile(scatterdata$x) + c(0, 0, 
    0, 0, 0.1), function(i) scatterdata$x < i)), levels = 1:4, labels = c("high", 
    "medhigh", "medlow", "low"), ordered = TRUE)

s1 <- ggplot() + geom_point(data = scatterdata, aes(x = x, y = y)) + xlab("very long x axis label") + 
    ylab("very long y axis label") + ggtitle("Titles are awesome")
s1
```
...
s3 <- ggplot() + geom_point(data = scatterdata, aes(x = xnew, y = y, colour = class, 
    fill = class)) + scale_colour_manual(values = c("#FF0000", "#0000FF")) + 
    scale_fill_manual(values = c("#FF0000", "#0000FF")) + ggtitle("Manual colour/fill scales")
s3
```
...
s4 <- ggplot() + geom_point(data = scatterdata, aes(x = xnew, y = y, colour = xnew, 
    fill = xnew)) + ggtitle("Categorical colour/fill scales")
s4
```
...
s6 <- ggplot() + geom_point(data = scatterdata, aes(x = x, y = y, color = x * 
    y, fill = x * y)) + ggtitle("Continuous color scales")
s6
```
...
```r
s5 <- ggplot() + geom_jitter(data = scatterdata, aes(x = xnew, y = y, colour = class4, 
    fill = class4)) + ggtitle("geom_jitter")
s5
```
...
```r
library(plyr)
scatterdata2 <- data.frame(x = rnorm(1000, 0, 0.5), y = rnorm(1000, 0, 0.5))
scatterdata2$quad <- c(3, 4, 2, 1)[with(scatterdata2, (3 + sign(x) + 2 * sign(y))/2 + 
    1)]
scatterdata2$quad <- factor(scatterdata2$quad, labels = c("Q1", "Q2", "Q3", 
    "Q4"), ordered = TRUE)
scatterdata2 <- ddply(scatterdata2, .(quad), transform, str = sqrt(x^2 + y^2)/4)
scatterdata2.summary <- ddply(scatterdata2, .(quad), summarise, xmin = min(x), 
    xmax = max(x), ymin = min(y), ymax = max(y), xmean = mean(x), ymean = mean(y))
...
#' Interactive plots...
s7 <- ggplot() + geom_rect(data = scatterdata2.summary, aes(xmax = xmax, xmin = xmin, 
    ymax = ymax, ymin = ymin, colour = quad, fill = quad, clickSelects = quad), 
    alpha = 0.3) + geom_point(data = scatterdata2.summary, aes(x = xmean, y = ymean, 
    colour = quad, fill = quad, showSelected = quad), size = 5) + geom_point(data = scatterdata2, 
    aes(x = x, y = y), alpha = 0.15) + scale_colour_discrete(guide = "legend") + 
    scale_fill_discrete(guide = "legend") + scale_alpha_discrete(guide = "none") + 
...
#' Single alpha value
s8 <- ggplot() + geom_point(data = scatterdata2, aes(x = x, y = y, colour = quad, 
    fill = quad), alpha = 0.2) + geom_point(data = scatterdata2, aes(x = x, 
    y = y, colour = quad, fill = quad, clickSelects = quad, showSelected = quad), 
    alpha = 0.6) + guides(colour = guide_legend(override.aes = list(alpha = 1)), 
    fill = guide_legend(override.aes = list(alpha = 1))) + ggtitle("Constant alpha")
s8
```
...
#' Continuous alpha
s9 <- ggplot() + geom_point(data = scatterdata2, aes(x = x, y = y, colour = quad, 
    fill = quad), alpha = 0.2) + geom_point(data = scatterdata2, aes(x = x, 
    y = y, colour = quad, fill = quad, alpha = str, clickSelects = quad, showSelected = quad)) + 
    guides(colour = guide_legend(override.aes = list(alpha = 1)), fill = guide_legend(override.aes = list(alpha = 1))) + 
    scale_alpha(range = c(0.6, 1), guide = "none") + ggtitle("Continuous alpha")
s9
```
...
s10 <- ggplot() + geom_point(data = scatterdata2, aes(x = x, y = y, colour = quad, 
    fill = quad, alpha = quad)) + geom_point(data = scatterdata2, aes(x = x, 
    y = y, colour = quad, fill = quad, alpha = quad, clickSelects = quad, showSelected = quad)) + 
    guides(colour = guide_legend(override.aes = list(alpha = 1)), fill = guide_legend(override.aes = list(alpha = 1))) + 
...
s11 <- ggplot() + geom_point(data = scatterdata2, aes(x = x, y = y, colour = quad, 
    fill = quad, size = str), alpha = 0.5) + geom_point(data = scatterdata2, 
    aes(x = x, y = y, colour = quad, fill = quad, size = str, clickSelects = quad, 
        showSelected = quad), alpha = 0.3) + ggtitle("Scale Size")
s11
```
...
s12 <- ggplot() + geom_point(data = scatterdata2, aes(x = x, y = y, colour = quad, 
    fill = quad, size = str), alpha = 0.5) + scale_size_area() + ggtitle("Scale Area")
s12
```
...
gg2animint(list(s7 = s7, s8 = s8, s9 = s9, s10 = s10, s11 = s11, s12 = s12), 
    out.dir = "geoms/interactivepoints")
```
