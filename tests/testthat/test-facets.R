...
test_that("facet_grid() strip labels are placed correctly", {
  ...
  # there should one be one 'top_strip' group (and we want kids)
  ...
  expect_equal(transforms, c("translate(108.5,17)rotate(0)", 
                             "translate(308.5,17)rotate(0)"))
  ...
  transforms <- as.character(sapply(attrs, "[[", "transform"))
  expect_match(transforms[[1]], "translate\\(389\\,66\\.6[0-9]+\\)rotate\\(90\\)")
  expect_equal(transforms[[2]], "translate(389,200)rotate(90)")
  expect_match(transforms[[3]], "translate\\(389\\,333\\.3[0-9]+\\)rotate\\(90\\)")
})
