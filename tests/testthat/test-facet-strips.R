...
test_that("facet_grid() strip labels are placed correctly", {
  ...
  # this produces a lot of false positives (if we mess with spacing of things)
  #expect_transform(transforms[1], c(118.5, 17, 0), context = c("translate", "rotate"))
  #expect_transform(transforms[2], c(318.5, 17, 0), context = c("translate", "rotate"))
  ...
  transforms <- as.character(sapply(attrs, "[[", "transform"))
  # this produces a lot of false positives (if we mess with spacing of things)
  #expect_transform(transforms[1], c(408.5, 90, 90), context = c("translate", "rotate"))
  #expect_transform(transforms[2], c(408.5, 222, 90), context = c("translate", "rotate"))
  #expect_transform(transforms[3], c(408.5, 356, 90), context = c("translate", "rotate"))
})
